import type { ResumeWorkExperienceItem } from '@/lib/candidate-resume';
import { formatTimelineDate } from '@/lib/candidate-resume';

export default function CandidateExperienceList({
  items,
}: {
  ...
  return (
    <section className="rounded-[24px] border border-brand-green/10 bg-white p-6 shadow-sm sm:p-7">
      <h2 className="text-xl font-bold text-brand-dark">Experience</h2>

      {items.length > 0 ? (
        <div className="mt-5 space-y-4">
          {items.map((item, index) => (
            <article key={`${item.role || 'role'}-${index}`} className="rounded-2xl border border-gray-100 bg-gray-50/70 p-5">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h3 className="text-lg font-semibold text-brand-dark">{item.role || 'Role not specified'}</h3>
