        ...
        if (id) {
          ...
          if (expandedOrg?.name) {
            ...
            try {
              ...
              // Keep fallback label if org fetch isn't available.
            }
          }
        }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      {canViewHiring ? (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        return (
                          <tr key={app.id} className="hover:bg-brand-green/5 transition-colors">
                            <td className="px-6 py-4 font-medium text-gray-900">
                              {applicant ? `${applicant.firstName || ''} ${applicant.lastName || ''}`.trim() || 'Unknown Candidate' : 'Unknown Candidate'}
                            </td>
                            ...
                            <td className="px-6 py-4 text-right">
                              <Link href={`/org/applications/${app.id}`} className="text-brand-green hover:text-green-800 font-medium">Review</Link>
                            </td>
                          </tr>
                        );
                      })}
            ...
            <div className="lg:col-span-1 space-y-6">
              {/* Active Jobs List */}
              <div className="bg-white border border-brand-green/10 rounded-2xl shadow-sm p-6">
                <h2 className="text-lg font-bold text-brand-dark mb-4">Latest Jobs</h2>
                {recentJobs.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {recentJobs.map(job => (
                      <div key={job.id} className="flex items-center justify-between pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                        <div>
                          <p className="font-medium text-brand-dark truncate max-w-[180px]">{job.role}</p>
                          <p className="text-xs text-gray-500">{job.type}</p>
                        </div>
                        <Link href={`/org/manage-jobs/${job.id}`} className="text-xs bg-brand-green/10 text-brand-green px-2 py-1 rounded hover:bg-brand-green/15">
                          Edit
                        </Link>
                      </div>
                    ))}
                ...
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <Link href="/org/manage-jobs" className="text-sm text-brand-green hover:text-green-800 font-medium block text-center">
                    Manage All Jobs
                  </Link>
                </div>
              </div>
              ...
              <div className="bg-white border border-brand-green/10 rounded-2xl shadow-sm p-6">
                ...
                <div className="space-y-3">
                  {canViewTeam(memberRole) && (
                    <Link href="/org/team" className="flex items-center gap-3 p-3 rounded-xl hover:bg-brand-green/5 transition-colors text-gray-700">
                      <div className="bg-brand-green/10 p-2 rounded-md text-brand-green">
                        <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" /></svg>
                      </div>
                      <span className="font-medium text-sm">
                        {canManageTeam(memberRole) ? 'Invite Team Member' : 'View Team'}
                      </span>
                    </Link>
                  )}
                  {canManageOrganization(memberRole) && (
                    <Link href="/org/organization-settings" className="flex items-center gap-3 p-3 rounded-xl hover:bg-brand-green/5 transition-colors text-gray-700">
                      <div className="bg-brand-green/10 p-2 rounded-md text-brand-green">
                  ...
                  {canAccessBilling(memberRole) && (
                    <Link href="/org/billing" className="flex items-center gap-3 p-3 rounded-xl hover:bg-brand-green/5 transition-colors text-gray-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-white border border-brand-green/10 rounded-2xl shadow-sm p-8">
            <h2 className="text-xl font-bold text-brand-dark">Organization Home</h2>
            <p className="mt-2 text-gray-600">
              You are signed in to <span className="font-semibold text-gray-800">{orgName}</span>.
              Your current role gives you access to account and organization resources, while hiring tools stay limited to recruiting roles.
            </p>
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              <div className="rounded-xl border border-brand-green/10 bg-brand-green/5 p-5">
                <p className="text-sm font-semibold text-brand-dark">{primaryOrgLinkLabel}</p>
                <p className="mt-1 text-sm text-gray-600">Open the primary management page available to your organization role.</p>
                <Link href={primaryOrgLink} className="mt-4 inline-block text-sm font-medium text-brand-green hover:text-green-800">
                  Open
                </Link>
              </div>
              {canAccessBilling(memberRole) && (
                <div className="rounded-xl border border-yellow-200 bg-yellow-50 p-5">
                  <p className="text-sm font-semibold text-gray-900">Billing Access</p>
                  <p className="mt-1 text-sm text-gray-600">Manage credit purchases and review payment history.</p>
                  <Link href="/org/billing" className="mt-4 inline-block text-sm font-medium text-brand-green hover:text-green-800">
                    Open billing
                  </Link>
                </div>
              )}
          ...
          <div className="bg-white border border-brand-green/10 rounded-2xl shadow-sm p-6">
            ...
            <div className="space-y-3">
              {canViewTeam(memberRole) && (
                <Link href="/org/team" className="block rounded-xl border border-brand-green/10 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-brand-green/5">
                  {canManageTeam(memberRole) ? 'Team Management' : 'Team Directory'}
                </Link>
              )}
              {canManageOrganization(memberRole) && (
                <Link href="/org/organization-settings" className="block rounded-xl border border-brand-green/10 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-brand-green/5">
                  Organization Settings
                </Link>
              )}
              {canAccessBilling(memberRole) && (
                <Link href="/org/billing" className="block rounded-xl border border-brand-green/10 px-4 py-3 text-sm font-medium text-gray-700 hover:bg-brand-green/5">
                  Billing
                </Link>
              )}
            </div>
